...
export default function BreedEncyclopedia() {
  ...
  const filtered = ALL_BREEDS.filter(b => {
    const matchSearch = b.breed.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="enc-container">
      ...
      <div className="detail-hero" style={{ background: typeStyle.bg }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "8px", marginTop: "8px", flexWrap: "wrap" }}>
            <span className="badge" style={{ background: typeStyle.bg, color: typeStyle.color, border: `1px solid ${typeStyle.color}` }}>
              {breed.type}
            </span>
            <span className="badge" style={{ background: heatStyle.bg, color: heatStyle.color }}>
              🌡️ {breed.heat_tolerance} heat tolerance
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="accuracy-bar-wrap">
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
          <span style={{ fontSize: "13px", color: "#888" }}>Model Accuracy</span>
          <span style={{ fontSize: "13px", fontWeight: "700", color: "#1565C0" }}>{breed.modelAccuracy}%</span>
        </div>
      ...
      <div className="detail-section">
        ...
        <div className="detail-card">
          <p className="health-status-text" style={{ color: "#2D6A4F" }}>
            ✅ {breed.health_status}
          </p>
